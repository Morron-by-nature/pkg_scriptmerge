...
//	$string = preg_replace("/(\/\*@)([^\n\r]*?)(@\*\/)/", "YUTTy$2ZUTTx", $string);				
    ...
    $string = preg_replace("/[\/][\/]+(ZQerTVQerT\s*[\"]\s*[\+])/", "WQerT", $string);
	$string = preg_replace("/[\/][\/]+(\*\/ZQerTVQerT\s*[\"]\s*[\+])/", "YQerT", $string);
...
// (Mark) Regex Find all "  Mark with = AwTc  and  CwRc // special cahacers are:  . \ + * ? ^ $ [ ] ( ) { } < > = ! | : " '
    $string = preg_replace("/(?!([\r\n]))(\+|\?|&|\=\s*[\[]?|\|\||\(|\!|,|return(?!(\/[a-zA-Z]+))|\:)(?!(\s*\/\/|\n|\/\*[^\r\n\*]*?\*\/))(\s*\/([\^]?))(?!([\^\r\n\*\/]|[\*]))(?!(\<\!\-\-))(([^\]\)\}\*;,g&\.\"\']?\s*)(?=([\]\)\}\*;,g&\.\/\"\']))?)((.*)(([\w\W])(\/\s*)(?=(\]))|([\w\W])(\/\s*)(?=(\}))|([^\\\\])(\/\s*)(?=(\)))|([\w\W])(\/\s*)(?=([i][g]?[\W]))|([\w\W])(\/\s*)(?=([g][i]?[\W]))|([^\*])(\/\s*)(?=(\,))|([^\\\\]|[\/])(\/\s*)(?=(;))|([\w\W])(\/\:\s)(?!([@\[\)\(\}\{\.,#%\+-\=`~\*&\^;\:\'\"]))|([^\\\\])(\/\s*)(?=(\.[^\/]))|([^\\\\])(\/\s*)(?=([\r\n]\s*[;\.,\)\}\]]\s*[^\/]|[\r\n]\s*([i][g]?[\W])|[\r\n]\s*([g][i]?[\W]))))|([^\\\\])(\/\s*)(?=([;\.,\)\}\]]\s*[^\/]|([i][g]?[\W])|([g][i]?[\W]))))/", "$2$3$5AwTc$7$8$10$13$15$18$21$24$27$30$33$36$39$42$47CwRc$16$19$22$25$28$31$34$37$40$43$48", $string);
    ...
    // (star slash) or (slash star) 1 sentence! Protect! With pDdYX and ODdPK
    do {$string = preg_replace('/((\")?YuKt)([^\r\n]*?)(\/)(\*)(?=([^\r\n]*?ZuKd))/', '$1$3pDdYX', $string, 1, $count);} while ($count);
    do {$string = preg_replace('/((\")?YuKt)([^\r\n]*?)(\*)(\/)(?=([^\r\n]*?ZuKd))/', '$1$3ODdPK', $string, 1, $count);} while ($count);
    // (slash slash) 1 sentence! Protect with: qDdXX
    do {$string = preg_replace('/((\")?YuKt)([^\r\n]*?)(\/)(\/)(?=([^\r\n]*?ZuKd))/', '$1$3qDdXX', $string, 1, $count);} while ($count); 
    //---------------------------------------------------------------------------------------------------------
    //---------------------------------------------------------------------------------------------------------
    ...
    do {$string = preg_replace("/(\"[^\r\n\;\,\"]*?)(\/)(\*)([^\r\n;\,\"]*?)(\")/", "$1pDdYX$4$5", $string, 1, $count);} while ($count);   // open
    ...
    $string = preg_replace('/ZQerT/', '\\\\r', $string);   // Restore \n"
	    $string = preg_replace('/LDdXX\//', '\/\/', $string);   // Restore \n"  
    ////---------------------------------------------------------------------------------------------------------
    ////---------------------------------------------------------------------------------------------------------
    //// Remove all markings!
